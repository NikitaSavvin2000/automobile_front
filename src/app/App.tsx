...
export default function App() {
  ...
  const handleAddRecord = (newRecord: NewRecord) => {
    const record: HistoryRecord = {
      ...
      time: new Date(newRecord.date).toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' }),
      ...
      <div className="h-screen w-full max-w-md mx-auto bg-secondary/30 flex flex-col">
        ...
        {showCarBlock && (
          <div className="p-4 pb-2">
            <div className={`bg-white dark:bg-card rounded-2xl shadow-sm transition-all duration-300 ${
              isScrolled ? 'p-3' : 'p-4'
            }`}>
              ...
              <div className={`flex items-center justify-between transition-all duration-300 ${
                ...
                <div className="flex items-center gap-3">
                  <div className={`bg-primary/10 rounded-xl flex items-center justify-center transition-all duration-300 ${
                    isScrolled ? 'p-2' : 'p-3'
                  }`}>
                    <CarIcon className={`text-primary transition-all duration-300 ${
                  ...
                  <div>
                    ...
                    }`}>{selectedCar.brand} {selectedCar.model}</h2>
                    {!isScrolled && (
                      <p className="text-xs text-muted-foreground">{selectedCar.year} год {selectedCar.color && `• ${selectedCar.color}`}</p>
                    )}
                  </div>
                </div>

                <div className={`bg-secondary rounded-xl flex items-center gap-2 transition-all duration-300 ${
                  isScrolled ? 'px-3 py-2' : 'px-4 py-2.5'
                }`}>
                  <Gauge className={`text-primary transition-all duration-300 ${
