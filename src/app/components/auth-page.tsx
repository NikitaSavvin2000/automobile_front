...
interface AuthPageProps {
  ...
  setTokens: (tokens: { access_token: string | null; refresh_token: string | null }) => void;
}

export function AuthPage({ onLogin, onChangePassword, setTokens }: AuthPageProps) {
  ...
  const nameInputProps = useShakeInput(name, setName, errors.name || false, () =>
  ...
  const codeInputProps = useShakeInput(
    verificationCode,
    setVerificationCode,
    errors.code || false,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const result = await registerUser(email, password, Number(verificationCode));
        if (!result) {
  ...
  return (
    <div className="min-h-screen w-full max-w-md mx-auto bg-gradient-to-b from-primary/5 to-secondary/30 flex flex-col items-center justify-center p-4">
      ...
      <div className="mb-8 flex flex-col items-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">История обслуживания вашего автомобиля</p>
      </div>
      ...
      <p className="text-xs text-muted-foreground text-center mt-6 px-8">
        Нажимая "{isLogin ? "Войти" : "Зарегистрироваться"}", вы соглашаетесь с условиями
        использования
      </p>
    </div>
  );
